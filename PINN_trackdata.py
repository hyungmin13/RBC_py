...
if __name__ == "__main__":
    ...
    path = '/scratch/hyun/RBC_challenge_data/fitted_0_05ppp_train/'
    timeskip = 1
    track_limit = 424070
    frequency = 33
...
# %%
    uni_t, _ = np.unique(train_data['pos'][:,0], return_counts=True)
    train_data['pos'] = np.concatenate([train_data['pos'][:,0:1]*domain_range['t'][1],
                                        train_data['pos'][:,1:2]*domain_range['x'][1]*1000**2,
                                        train_data['pos'][:,2:3]*domain_range['y'][1]*1000**2,
                                        train_data['pos'][:,3:4]*domain_range['z'][1]*1000**2],1)
# %%
    X = np.fft.fft(train_data['pos'][:,1])
    freqsx = np.fft.fftfreq(train_data['pos'][:,1].shape[0], d=(uni_t[1]-uni_t[0]))
    Y = np.fft.fft(train_data['pos'][:,2])
    freqsy = np.fft.fftfreq(train_data['pos'][:,2].shape[0], d=(uni_t[1]-uni_t[0]))
    Z = np.fft.fft(train_data['pos'][:,3])
    freqsz = np.fft.fftfreq(train_data['pos'][:,3].shape[0], d=(uni_t[1]-uni_t[0]))
    X = np.abs(X)
    Y = np.abs(Y)
    Z = np.abs(Z)
#%%
    import matplotlib.pyplot as plt
    plt.loglog(freqsx[:len(freqsx)//2],X[:len(freqsx)//2])
    plt.loglog(freqsy[:len(freqsy)//2],Y[:len(freqsy)//2])
    plt.loglog(freqsz[:len(freqsz)//2],Z[:len(freqsz)//2])
    plt.show()
# %%
    plt.plot(freqsx[:len(freqsx)//2]/np.max(freqsx[:len(freqsx)//2]),X[:len(freqsx)//2]/np.max(X[:len(freqsx)//2]))
    plt.plot(freqsy[:len(freqsy)//2]/np.max(freqsy[:len(freqsy)//2]),Y[:len(freqsy)//2]/np.max(Y[:len(freqsx)//2]))
    plt.plot(freqsz[:len(freqsz)//2]/np.max(freqsz[:len(freqsz)//2]),Z[:len(freqsz)//2]/np.max(Z[:len(freqsx)//2]))
    plt.xscale('log')
    plt.yscale('log')
    plt.show()
# %%
