...
def equ_func(all_params, g_batch, cotangent1, cotangent2, model_fns):
    def u_t(batch):
...
def equ_func2(all_params, g_batch, cotangent, model_fns):
    ...
    return out, out_t
def get_cotangent(direction, size):
    d = {"t": 0, "x": 1, "y": 2, "z": 3}
    vec = [0.0] * 4
    vec[d[direction]] = 1.0
    return jnp.tile(jnp.array([vec]), (size, 1))

def grad(model_fns, g_batch, cot):
    return jax.jvp(model_fns, (g_batch,), (cot,))[1]

def grad2(model_fns, g_batch, cot1, cot2):
    return jax.jvp(lambda x: grad(model_fns, x, cot1), (g_batch,), (cot2,))[1]
...
def PINN_loss(dynamic_params, all_params, g_batch, particles, particle_vel, model_fns):
    all_params["network"]["layers"] = dynamic_params
    weights = all_params["problem"]["loss_weights"]
    cot_t = get_cotangent("t", g_batch.shape[0])
    cot_x = get_cotangent("x", g_batch.shape[0])
    cot_y = get_cotangent("y", g_batch.shape[0])
    cot_z = get_cotangent("z", g_batch.shape[0])
    out, out_t = equ_func2(all_params, g_batch, jnp.tile(jnp.array([[1.0, 0.0, 0.0, 0.0]]),(g_batch.shape[0],1)),model_fns)
    _, out_tx = equ_func(all_params, g_batch, jnp.tile(jnp.array([[1.0, 0.0, 0.0, 0.0]]),(g_batch.shape[0],1)),
                             jnp.tile(jnp.array([[0.0, 1.0, 0.0, 0.0]]),(g_batch.shape[0],1)),model_fns)
    _, out_ty = equ_func(all_params, g_batch, jnp.tile(jnp.array([[1.0, 0.0, 0.0, 0.0]]),(g_batch.shape[0],1)),
                             jnp.tile(jnp.array([[0.0, 0.0, 1.0, 0.0]]),(g_batch.shape[0],1)),model_fns)
    _, out_tz = equ_func(all_params, g_batch, jnp.tile(jnp.array([[1.0, 0.0, 0.0, 0.0]]),(g_batch.shape[0],1)),
                             jnp.tile(jnp.array([[0.0, 0.0, 0.0, 1.0]]),(g_batch.shape[0],1)),model_fns)
    out_x, out_xx = equ_func(all_params, g_batch, jnp.tile(jnp.array([[0.0, 1.0, 0.0, 0.0]]),(g_batch.shape[0],1)),
                             jnp.tile(jnp.array([[0.0, 1.0, 0.0, 0.0]]),(g_batch.shape[0],1)),model_fns)
    _, out_xy = equ_func(all_params, g_batch, jnp.tile(jnp.array([[0.0, 1.0, 0.0, 0.0]]),(g_batch.shape[0],1)),
                             jnp.tile(jnp.array([[0.0, 0.0, 1.0, 0.0]]),(g_batch.shape[0],1)),model_fns)
    _, out_xz = equ_func(all_params, g_batch, jnp.tile(jnp.array([[0.0, 1.0, 0.0, 0.0]]),(g_batch.shape[0],1)),
                             jnp.tile(jnp.array([[0.0, 0.0, 0.0, 1.0]]),(g_batch.shape[0],1)),model_fns)
    _, out_yz = equ_func(all_params, g_batch, jnp.tile(jnp.array([[0.0, 0.0, 1.0, 0.0]]),(g_batch.shape[0],1)),
                             jnp.tile(jnp.array([[0.0, 0.0, 0.0, 1.0]]),(g_batch.shape[0],1)),model_fns)
    out_y, out_yy = equ_func(all_params, g_batch, jnp.tile(jnp.array([[0.0, 0.0, 1.0, 0.0]]),(g_batch.shape[0],1)),
                             jnp.tile(jnp.array([[0.0, 0.0, 1.0, 0.0]]),(g_batch.shape[0],1)),model_fns)
    out_z, out_zz = equ_func(all_params, g_batch, jnp.tile(jnp.array([[0.0, 0.0, 0.0, 1.0]]),(g_batch.shape[0],1)),
                             jnp.tile(jnp.array([[0.0, 0.0, 0.0, 1.0]]),(g_batch.shape[0],1)),model_fns)


    p_out = model_fns(all_params, particles)

    ur, vr, wr, Tr = all_params["data"]["u_ref"], all_params["data"]["v_ref"], all_params["data"]["w_ref"], 13
    get = lambda k: all_params["data"]["domain_range"][k][1]
    norm = lambda val, ref, scale1, scale2: ref * val / scale1 / scale2

    u = norm(out[:, 0:1], ur, 1, 1)
    v = norm(out[:, 1:2], vr, 1, 1)
    w = norm(out[:, 2:3], wr, 1, 1)

    ut = norm(out_t[:, 0:1], ur, get("t"), 1)
    vt = norm(out_t[:, 1:2], vr, get("t"), 1)
    wt = norm(out_t[:, 2:3], wr, get("t"), 1)

    ux = norm(out_x[:, 0:1], ur, get("x"), 1)
    uy = norm(out_y[:, 0:1], ur, get("y"), 1)
    uz = norm(out_z[:, 0:1], ur, get("z"), 1)

    vx = norm(out_x[:, 1:2], vr, get("x"), 1)
    vy = norm(out_y[:, 1:2], vr, get("y"), 1)
    vz = norm(out_z[:, 1:2], vr, get("z"), 1)

    wx = norm(out_x[:, 2:3], wr, get("x"), 1)
    wy = norm(out_y[:, 2:3], wr, get("y"), 1)
    wz = norm(out_z[:, 2:3], wr, get("z"), 1)

    Tx = norm(out_x[:, 3:4], Tr, get("x"), 1)
    Ty = norm(out_y[:, 3:4], Tr, get("y"), 1)

    uty = norm(out_ty[:, 0:1], ur, get("t"), get("y"))
    utz = norm(out_tz[:, 0:1], ur, get("t"), get("z"))
    vtx = norm(out_tx[:, 1:2], vr, get("t"), get("x"))
    vtz = norm(out_tz[:, 1:2], vr, get("t"), get("z"))
    wtx = norm(out_tx[:, 2:3], wr, get("t"), get("x"))
    wty = norm(out_ty[:, 2:3], wr, get("t"), get("y"))
    uxy = norm(out_xy[:, 0:1], ur, get("y"), get("x"))
    uxz = norm(out_xz[:, 0:1], ur, get("z"), get("x"))
    uyz = norm(out_yz[:, 0:1], ur, get("z"), get("y"))
    vxy = norm(out_xy[:, 1:2], vr, get("y"), get("x"))
    vxz = norm(out_xz[:, 1:2], vr, get("z"), get("x"))
    vyz = norm(out_yz[:, 1:2], vr, get("z"), get("y"))
    wxy = norm(out_xy[:, 2:3], wr, get("y"), get("x"))
    wxz = norm(out_xz[:, 2:3], wr, get("z"), get("x"))
    wyz = norm(out_yz[:, 2:3], wr, get("z"), get("y"))


    uxx = norm(out_xx[:, 0:1], ur, get("x"), get("x"))
    uyy = norm(out_yy[:, 0:1], ur, get("y"), get("y"))
    uzz = norm(out_zz[:, 0:1], ur, get("z"), get("z"))
    vxx = norm(out_xx[:, 1:2], vr, get("x"), get("x"))
    vyy = norm(out_yy[:, 1:2], vr, get("y"), get("y"))
    vzz = norm(out_zz[:, 1:2], vr, get("z"), get("z"))
    wxx = norm(out_xx[:, 2:3], wr, get("x"), get("x"))
    wyy = norm(out_yy[:, 2:3], wr, get("y"), get("y"))
    wzz = norm(out_zz[:, 2:3], wr, get("z"), get("z"))


    L_uz = grad(lambda x: uxx + uyy + uzz, g_batch, get_cotangent("z", g_batch.shape[0]))
    L_uy = grad(lambda x: uxx + uyy + uzz, g_batch, get_cotangent("y", g_batch.shape[0]))
    L_vx = grad(lambda x: vxx + vyy + vzz, g_batch, get_cotangent("x", g_batch.shape[0]))
    L_vz = grad(lambda x: vxx + vyy + vzz, g_batch, get_cotangent("z", g_batch.shape[0]))
    L_wx = grad(lambda x: wxx + wyy + wzz, g_batch, get_cotangent("x", g_batch.shape[0]))
    L_wy = grad(lambda x: wxx + wyy + wzz, g_batch, get_cotangent("y", g_batch.shape[0]))
    ...
    loss_vor1 = wty - vtz + u*(wxy - vxz) + v*(wyy - vyz) + w*(wyz - vzz) \
                - wy*ux + vz*ux - uz*uy + wx*uy -vx*uz + uy*uz \
                - all_params["data"]["viscosity"]*(L_wy - L_vz) + 0.00342086*9.81*Tx
    loss_vor2 = utz - wtx + u *(uxz - wxx) + v*(uyz - wxy) + w*(uzz - wxz) \
               - wy*vx + vz*vx - uz*vy + wx*vy - vx*vz + uy*vz \
               - all_params["data"]["viscosity"]*(L_uz - L_wx) - 0.00342086*9.81*Ty
    loss_vor3 = vtx - uty + u*(vxx - uxy) + v*(vxy - uyy) + w*(vxz - uyz) \
                - wy*wx + vz*wx - uz*wy + wx*wy - vx*wz + uy*wz \
                - all_params["data"]["viscosity"]*(L_vx - L_uy)

    loss_vor1 = jnp.mean(loss_vor1**2)
    loss_vor2 = jnp.mean(loss_vor2**2)
    loss_vor3 = jnp.mean(loss_vor3**2)

    total_loss = weights[0]*loss_u + weights[1]*loss_v + weights[2]*loss_w + \
                 weights[3]*loss_con + weights[4]*loss_vor1 + weights[5]*loss_vor2 + \
                 weights[6]*loss_vor3 #+ weights[7]*(loss_u_b + loss_v_b + loss_w_b)
    return total_loss
...
if __name__=="__main__":
    ...
    # Set Data params
    path = 'RBC_challenge_data/fitted_0_05ppp_train/'
    timeskip = 1
    track_limit = 424070
    viscosity = 15.314*10**(-6)
